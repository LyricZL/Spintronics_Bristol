function spin_wave_simulation_adjusted
    % 参数设置
    ...
    % 初始条件
    M0 = cellfun(@(x) [x; x; 0], num2cell(Ms), 'UniformOutput', false);
...
function stt_amplification_simulation_adjusted
    % 参数设置
    gamma = 1.76e11;  % 磁旋比 (rad/s/T)
    H_ext = [0; 0; 1e3];  % 外部磁场 (A/m)
    tspan = [0 1e-9];  % 时间范围
...
function spin_pumping_simulation_adjusted
    % 参数设置
    gamma = 1.76e11;  % 磁旋比 (rad/s/T)
    H_ext = [0; 0; 1e3];  % 外部磁场 (A/m)
    tspan = [0 1e-9];  % 时间范围
